...
class SessionManager:
    ...
    def export_uncracked_hashes (self, session_id, save_as):
        # First get the session.
        session = self.get(session_id=session_id)[0]
        if os.path.isfile(save_as):
            os.remove(save_as)

        command = self.hashcat.build_export_uncracked_hashes_command_line(
            self.session_filesystem.get_hashfile_path(session.user_id, session_id),
            self.session_filesystem.get_potfile_path(session.user_id, session_id),
            save_as,
            session.hashcat.contains_usernames,
    ...
    def download_file(self, session_id, which_file):
        ...
        if which_file == 'cracked':
            file = self.session_filesystem.get_potfile_path(session.user_id, session_id)
        ...
        elif which_file == 'uncracked':
            file = self.session_filesystem.get_uncrackedfile_path(session.user_id, session_id)
            self.export_uncracked_hashes(session_id, file)
            save_as = save_as + '.uncracked.txt'
    ...
    def get_uncracked_hashes(self, session_id):
        session = self.get(session_id=session_id)[0]
        file = self.session_filesystem.get_uncrackedfile_path(session.user_id, session_id)
        if os.path.isfile(file):
            os.remove(file)
